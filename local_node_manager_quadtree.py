...
class Local_node_manager:
    ...
    def update_local_graph(self, robot_location, local_frontiers, local_map_info, extended_local_map_info):
        ...
        for coords in local_node_coords:
            node = self.check_node_exist_in_dict(coords)
            if node is None:
                self.add_node_to_dict(coords, local_frontiers, extended_local_map_info, self.target)
            else:
                pass
    ...
    def get_all_node_graph(self, robot_location, robot_locations):
        ...
        for node in self.local_nodes_dict.__iter__():
            all_node_coords.append(node.data.coords)  #nodes are in meters coordinate
        ...
        for i, coords in enumerate(all_node_coords):
            ...
            direction_vector.append(node.direction_vect)
        ...
        return all_node_coords, utility, guidepost, occupancy, adjacent_matrix, current_index, neighbor_indices, direction_vector
...
class Local_node:
    ...
    def get_direction_vector(self):
        dx = self.target[0] - self.coords[0]
        dy = self.target[1] - self.coords[1]
        mag = (dx**2 + dy**2)** .5
        if mag != 0:
            dx = dx / mag
            dy = dy / mag
